...
def main():
    ...
    title_template = ChatPromptTemplate(
        [
            ("system", "You are a helpful assistant that generates article content based on a given topic."),
    ...
    if topic:
        response = titlechain.invoke( {"topic": topic, "language": language},config={'callbacks': [ConsoleCallbackHandler()]})
        st.write(response)
