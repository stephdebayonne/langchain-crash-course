...
def main():
    ...
    title_template = ChatPromptTemplate.from_messages(
        [
            ("system", "You are a helpful assistant that generates article titles based on a given topic."),
            ("human", "Generate an article title for the following topic: {topic} in {language}" )
        ]
    )
    ...
    if topic:
        response = llm.invoke(title_template.format_messages(topic=topic, language=language)   )
        st.write(response)
